...
Additionally, we look up some actual game histories and calculate the actual cross-team winning probabilities, as described below.

#### MLB Game Logs data

Major League Baseball (MLB) is the oldest and largest professional baseball organization in U.S and Canada. It has 30 teams playing in 2 major leagues: National League (NL) and American League (AL). NL and AL were seperate legal entitles formed in 1876 and 1901 respectilvely, and they cooperated to found MLB in 1903.  We obtain free MLB game logs from www.retrosheet.org. Since game statistics are highly time-sensitive (a strong team may lose its dominance in 1 season), we use recent 3 years' game log (2016, 2017 and 2018) to build the winning probability matrix $M$. 
...
The dataset (game logs in recent 3 years) has a total of $`r nrow(gl)`$ games in record. We can see one main problem here is although we have this many games in record. Many team does not have many chances to play some other teams. This is because teams basically play in seperate leagues (NL and AL) and seperate region in season, and most of games happen within one region and one league. So we have many records of some teams vs some teams, but less for other teams. In short, this dataset is highly unbalanced. Further discussion about this problem can be found in [Results]. 

```{r}
#total possible combination of team vs team without considering visiting and home team
30^2/2
#number of (team vs team) game log with less than 10 game logs in record
sum(alltotal<10)/2
```
...
### Team Power Estimation via Resampling
From the tournament results we generated, we have several possible estimator to estimate the real team power. We can use Bootstrap resampling to get more information about that estimator. 
...
#ranks of each team
ranks <- lapply(1:n, getRank, res = res)
...
##### Appropriate seeding
Then we try using our seeding function to simulation the tournament. We can see in this case, the difference between teams increases. And the situation of team with lower team power having higher rank becomes less. Generally speaking, with appropriate seeding, average team rank represents the team power better. 
```{r,echo=F}
#With our seeding
t <- tournament_seeding(n) # tournament seeding
res = replicate(nSim, simTournament(
...
### Parameter Analysis via Resampling
...
We consider teams 1 to 8 (team 1 being the strongest). For each parameter setting, we simulate the tournament 1000 times using random seeding, and accordingly obtain a vector of 1000 winner team indices. To reduce the influence of chance in generating a particular cross-team winning probability matrix, we regenerate it and also update the seeding in each simulation. We use the portion of 1 in the vector as the estimator. We use Bootstrap to get a distribution of this estimator.
...
```{r, echo=FALSE, out.width = "50%", fig.pos="h", fig.align="center", fig.cap="Figure 3"}
## Results
...
### Seeding Effect Analysis via Resampling
...
```{r, echo=FALSE, out.width = "50%", fig.pos="h", fig.align="center", fig.cap="Figure 4"}
# Random seeding
genSeeds <- function() {
    return(random_seeding(numTeams))
}
...
### General Helper Functions for Analysis
```{r}
#' Get ranks of a team in tournament simulation
#' @author Ziqin Xiong
#' 
